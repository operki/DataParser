    ...
    internal HttpDataFactory(HttpClientSettings settings)
    {
        ...
        if(settings.BaseUrl != null)
        {
            ...
            if(uriKind == UriKind.Relative)
                throw new Exception($"Can't init DataParsers.HttpClient with '{baseUri}', need absolute path");

            BaseUrl = baseUri.GetLeftPart(UriPartial.Authority);
            if(baseUri.Scheme != Uri.UriSchemeHttps)
            {
                if(settings.OnlyHttps)
                    throw new Exception($"Can't init DataParsers.HttpClient with '{baseUri}', only {Uri.UriSchemeHttps} allowed with parameters");
                if(settings.Proxy != null)
                    throw new Exception($"Can't init DataParsers.HttpClient with '{baseUri}', only {Uri.UriSchemeHttps} allowed with proxy");
                if(baseUri.Scheme != Uri.UriSchemeHttp)
                    throw new Exception($"Can't init DataParsers.HttpClient with '{baseUri}', only {Uri.UriSchemeHttp} and {Uri.UriSchemeHttps} allowed");
            }
        }
